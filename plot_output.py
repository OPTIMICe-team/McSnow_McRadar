...
if ('trajectories' not in experimentID) and ('trajectories' not in inputPath):
	...
	plot.plotconcHeightProf('Real',mcTable,inputPath,dicSettings)
	print('plotting number of sp')
	plot.plotconcHeightProf('Super',mcTable,inputPath,dicSettings,nz=nz)
	print('plotting PSD')
	...
	if os.path.exists(inputPath+McRadar_Outname):
		...
		if len(freq)==2:
			...
			plot.plotOverview(output,dicSettings,inputPath,dicSettings['wl'][0],dicSettings['wl'][1],wl3=dicSettings['wl'][2])
